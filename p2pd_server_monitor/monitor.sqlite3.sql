...
FOR EACH ROW
BEGIN
    DELETE FROM status WHERE table_type = 15 AND row_id = OLD.id;
END;
CREATE TRIGGER delete_status_on_imports
AFTER DELETE ON imports
FOR EACH ROW
BEGIN
    DELETE FROM status WHERE table_type = 16 AND row_id = OLD.id;
END;
CREATE TRIGGER delete_status_on_services
AFTER DELETE ON services
FOR EACH ROW
BEGIN
    DELETE FROM status WHERE table_type = 14 AND row_id = OLD.id;
END;
CREATE TRIGGER filter_secondary_stun_ips
BEFORE INSERT ON services
FOR EACH ROW
WHEN NEW.type = 3 AND NEW.proto = 2
BEGIN
    -- Count existing type=4, proto=2 rows in this group
    SELECT
    CASE
        WHEN (
            SELECT COUNT(*) 
        ...
        THEN RAISE(ABORT, 'Conflict: type 3 with same ip and af exists')
    END;
END;
